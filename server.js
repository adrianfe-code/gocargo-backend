/**
 * GoCargo – Backend de pagos con dLocal
...
// ─── COTIZAR (proxy a SendGround) ─────────
app.post('/api/quote', async (req, res) => {
  try {
    const r = await fetch(
      `${process.env.SENDGROUND_API}/c1/Orders/Quote`,
      {
        method: 'POST',
        headers: {
          'Content-Type':     'application/json',
          'Authorization':    `Bearer ${process.env.SENDGROUND_TOKEN}`,
          'X-Application-Id': process.env.SENDGROUND_APP_ID || '23',
        },
...
// ─── CREAR PEDIDO (proxy a SendGround) ────
app.post('/api/order', async (req, res) => {
  try {
    const r = await fetch(
      `${process.env.SENDGROUND_API}/c1/Orders`,
      {
        method: 'POST',
        headers: {
          'Content-Type':     'application/json',
...
app.post('/api/payment', async (req, res) => {
  ...
  const auth    = `V2-HMAC-SHA256, Signature: ${hmac}`;
