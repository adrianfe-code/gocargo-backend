...
// ─── DISTANCE (proxy a Google Maps) ───────
app.get('/api/distance', async (req, res) => {
  const { orig, dest } = req.query;
  if (!orig || !dest) return res.status(400).json({ error: 'orig y dest requeridos' });
  const key = process.env.GOOGLE_MAPS_KEY || 'AIzaSyDZCKACsysWaDYJlyWiE-xcdaLHm5uOoAA';
  try {
    const url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${orig}&destinations=${dest}&mode=driving&key=${key}`;
    const r = await fetch(url);
    const data = await r.json();
    if (data.status === 'OK' && data.rows[0]?.elements[0]?.status === 'OK') {
      const metros = data.rows[0].elements[0].distance.value;
      res.json({ km: metros / 1000 });
    } else {
